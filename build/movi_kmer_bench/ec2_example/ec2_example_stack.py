"""
Stack for granular (hardware-PMU) benchmarking of the Movi `movi-co` k-mer
coroutine query, on a bare-metal EC2 instance where perf PEBS sampling and
likwid uncore counters are fully available (unlike the Rockfish dev node, whose
perf_event_paranoid=2 blocks per-line sampling).
...
reads from S3, builds movi-co (RelWithDebInfo so perf maps to source lines), and
runs bench/run_bench.sh. Results are tarred up to S3.

Mirrors the conventions of build/pfp_thresh_builder.
...
class CdkEc2Stack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        cfn_key_pair = ec2.CfnKeyPair(
            ...
            key_name="movi-kmer-bench",
            tags=[CfnTag(key="application", value="movi-kmer-bench")],
        )

        # Credential model (same as build/bowtie): NO permanent credentials.
        ...
        # PERMANENT instance role. We do NOT create the role in CFN, because a
        # CDK-managed role is deleted on `cdk destroy`, and deleting a role that
        # is named in a cross-account trust policy invalidates that trust (it is
        # converted to a stale unique-id) -- forcing a manual trust re-save after
        # every redeploy. Instead the role is created ONCE, out of band, by
        # setup_permanent_role.sh, carrying its own sts:AssumeRole policy. Here we
        # only *reference* it (mutable=False => CDK never touches its policies),
        # so the S3UploadFromComputeRole trust is set once and never breaks.
        # CDK still creates an ephemeral instance profile wrapping it each deploy;
        # instance profiles are not named in trust policies, so that is harmless.
        ...
        user_data_script = '\n'.join([
            ...
            f"aws s3 cp --no-sign-request {config['ftab_s3_uri']} $BENCH/index/ftab.10.bin",
            reads_dl,

            # --- build movi-co (RelWithDebInfo => -O2 -g, so perf maps to source) ---
            ...
            "cmake --build . --target movi-co -j$(nproc)",
            ...
            f"export BENCH=$BENCH CO=$CO IDX=$BENCH/index READS_DIR=$BENCH/reads",
            f"export K={config['k']} FTABK={config['ftab_k']}",
        ...
        CfnOutput(self, "KeyPairId", value=cfn_key_pair.attr_key_pair_id,
                  description="id of the key pair", export_name="movi-bench-keypair-id")
        CfnOutput(self, "PublicIp", value=instance.attr_public_ip,
                  description="public ip of the instance", export_name="movi-bench-public-ip")
