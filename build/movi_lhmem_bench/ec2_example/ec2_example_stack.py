...
class CdkEc2Stack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        cfn_key_pair = ec2.CfnKeyPair(
            ...
            key_name="movi-lhmem-bench",
            tags=[CfnTag(key="application", value="movi-lhmem-bench")],
        )
        ...
        # reads are full S3 URIs (two prefixes); land each under reads/ by basename.
        reads = config['reads']
        reads_dl = '\n'.join(
            f"aws s3 cp --no-sign-request {uri} "
            f"/home/ec2-user/bench/reads/{uri.rsplit('/', 1)[1]}" for uri in reads
        )
        ...
        ftab_ks = ' '.join(str(k) for k in config['ftab_ks'])
        ...
        user_data_script = '\n'.join([
            ...
            f"export MIN_MEM_LEN={config['min_mem_length']} FTAB_KS=\"{ftab_ks}\"",
        ...
        CfnOutput(self, "KeyPairId", value=cfn_key_pair.attr_key_pair_id,
                  description="id of the key pair", export_name="movi-lhmem-bench-keypair-id")
        CfnOutput(self, "PublicIp", value=instance.attr_public_ip,
                  description="public ip of the instance", export_name="movi-lhmem-bench-public-ip")
